...
- Build the Sprint 9-4B annual quasi point-in-time fundamentals table for the 243 screener-relevant tickers from the existing FinanceClient annual extended-history path, with dedicated resumable run-state cache, REQUIRED_ITEMS v1 output, raw-VND units, and gate-input buildability measurements only.
