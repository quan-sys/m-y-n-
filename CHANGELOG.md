...
- Amend `docs/SPEC_SPRINT_8E_POPULATION.md` sections 6 and 7 before any code is written; the amendment note is section 12 of that file. Section 6 no longer requires the import blocks of `scripts/build_sprint6_fscore.py` and `scripts/build_sprint6_franchise.py` to keep their line count: that requirement contradicted section 3, which asks for two separate module-level functions and therefore two imported names where one stood before. Owner decision: section 3 wins, the function shape is chosen on readability rather than on holding a line number fixed, and the stale provenance literals in `scripts/build_sprint9_4b_annual_pit.py` are repaired from measured line numbers instead. Section 7 now permits four edited lines across two existing test files rather than one: the `DEF-6` tuple entry in `tests/test_repo_decision_guards.py`, plus the three `expected_count` keyword arguments in `tests/test_sprint5_market_cap.py` that removing the parameter breaks. Measured, not assumed: with the parameter removed and those three arguments deleted, that file runs 6 passed 0 failed and every assertion is byte-identical to `30c4aa9`. Process failure recorded: the first implementing prompt patched both findings into the prompt instead of into the specification; the implementing agent read both, found the disagreement, refused to branch and reported FAIL, which was correct and cost one round. Once a specification is committed, a new finding is patched into the specification, never into the prompt. No code is implemented by this entry.
- Implement DEF-6 from `docs/SPEC_SPRINT_8E_POPULATION.md`: replace all seven literal survivor-population guards with shared ticker-set consistency checks, without introducing a replacement count. Measured provenance repair in `scripts/build_sprint9_4b_annual_pit.py`: F-Score `153-446` becomes `154-447`; Franchise `118-248,398-411` becomes `119-249,396-409`.
- Narrow DEF-6 NA handling to the `ticker` column so blank ticker cells remain rejected without converting other numeric survivor columns and their missing values into text.
