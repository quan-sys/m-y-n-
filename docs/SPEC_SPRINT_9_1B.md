...
9-1B-6 MISSING EX-DATE. When a price-affecting event has a ratio or amount but no exright_date, use its record_date (or the nearest trading day at/after record_date) as the ex-date, and set adjustment_confidence = LOW for affected dates. If neither ex-date nor record_date exists, the event cannot be placed: flag the ticker UNPLACEABLE_EVENT, set its entire series adjustment_confidence = LOW, and list it in the report.
